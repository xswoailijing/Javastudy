...
class ClientGUIMessgeWindow extends ClientGUI {
	...
	TcpMessgeClient TcpMessgeClient;
	Clientthread clientthread;
	private static final long serialVersionUID = 325334855022493826L;
	public ClientGUIMessgeWindow(Client client, Clientinfo clientinfo) throws UnknownHostException, IOException {
		super();
		window_height=500;
		window_width=490;
		window_x=400;
		...
		this.client=client;
		this.TcpMessgeClient=new TcpMessgeClient(this.client);
		//添加消息接收后台线程
...
//		this.setLayout(new GridLayout(3, 3, 10, 10));
		...
		sendButton.addActionListener(new ActionListener() {	
		    ...
		    try {
				TcpMessgeClient.sendmessge("说:"+messgesendTextArea.getText());
		...
		exitButton.addActionListener(new ActionListener() {	
			public void actionPerformed(ActionEvent e) {				
			    try {
					TcpMessgeClient.sendmessge("退出聊天室!");
					Thread.sleep(1000);
					TcpMessgeClient.closeclient();
	...
	public void showmessge(String messge) {
		this.messgeshowTextArea.append(messge);
	}
	class Clientthread extends Thread{
		public void run() {	
			while (true) {
				try {				
					TcpMessgeClient.receive();
					showmessge(TcpMessgeClient.messge);
